...
/* Nº of active processes */
static int active_proc = 0;
...
static void copy_items_into_list( struct work_struct *work ) {	
	...
	while(buffer_size > 0){
		...
		new_node->data = num;
		// if to decide which list to add the nº , pairs odds
		list_add_tail(&(new_node->links), &mylist);
		buffer_size--;
	}
	/* acknowledge waiting process (modtimer_read) (Upper layer) */
...
static ssize_t modtimer_read(struct file *filp, char __user *buf, size_t len, loff_t *off) {
    ...
    int max_elems = MAX_KBUFF -2 / (sizeof(DATA_TYPE)+1) , count = 0; // 35 for test
...
static int fifoproc_open(struct inode *inode, struct file *file){
	...
	if(nr_proc_waiting == 1){
			
		if (down_interruptible(&sem)){return -EINTR;}
		/* nº of waiting processes +1 */		
		//active_proc += 1;
		//nr_proc_waiting +=1;
		
        while (nr_proc_waiting < 2){
        	//nr_proc_waiting++;
        	up(&sem);
